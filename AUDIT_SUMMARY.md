...
- PR1 checker hardened to upstream quality: detects entirely missing locale files, malformed JSON, invalid structures, non-string values, all 5 placeholder conventions used in the repo, stale keys, identical-to-English values; PEP8 + dataclasses; 9 unit tests pass; summary line per run.
- PR2: all 1299 mobile missing strings translated to Polish (EN->PL, tokens/markup preserved), followed by contextual QA of confirmed MT errors (tabela, wzorzec, obraz, odrzuć, przenieś). Mobile structural coverage: Document 100%, Presentation 100%, Spreadsheet 100%, Visio 100% (missing=0, placeholders=0). 34 values identical to EN remain (13/4/13/4) — mostly technical terms and short labels, listed for review.
...
## Status
- DONE: PR1 (checker+tests), PR2 (structural+translated mobile+QA corrections), PR3 (desktop parity+placeholder fixes+QA corrections), reports.
- PARTIAL: translation QA (diff-scoped linguistic/format QA completed with official Polish glossary cross-check; independent human review remains recommended), desktop embed/forms identical values.
